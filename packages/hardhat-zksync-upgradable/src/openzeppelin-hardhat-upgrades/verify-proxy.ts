...
import { callEtherscanApi, getEtherscanInstance, RESPONSE_OK, verifyAndGetStatus } from './utils/etherscan-api';
        ...
        if (origError instanceof BytecodeNotMatchArtifact || origError instanceof EventNotFound) {
            ...
            try {
                ...
                if (fallbackError.message.toLowerCase().includes('already verified')) {
                    ...
                    recordError(
                        `Failed to verify directly using hardhat verify: ${fallbackError.message}`,
                        errorReport,
                    );
                    return;
                }

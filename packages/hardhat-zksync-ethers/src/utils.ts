...
export function createProviders(hre: HardhatRuntimeEnvironment): {
    ethWeb3Provider: ethers.providers.Provider;
...
export async function findWalletFromAddress(
    ...
    if (!hre) {
        throw new Error('Hardhat runtime environment is required to find wallet from address');
    }
...
function _createDefaultEthProvider(): ethers.providers.Provider {

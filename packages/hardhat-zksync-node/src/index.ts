...
task(TASK_NODE_ZKSYNC, 'Starts a JSON-RPC server for zkSync node')
    .addOptionalParam('log', 'Logging level (error, warn, info, debug)', undefined, types.string)
    .addOptionalParam('logFilePath', 'Path to the file where logs should be written', undefined, types.string)
    .addOptionalParam('cache', 'Cache network request (none, disk, memory)', undefined, types.string)
    .addOptionalParam('cacheDir', 'Path to the directory where cache should be stored', undefined, types.string)
    .addFlag('resetCache', 'Reset cache before start')
    .addOptionalParam('fork', 'Fork from the specified network (testnet, mainnet)', undefined, types.string)
    .addOptionalParam('showSorageLogs', 'Show storage logs (none, read, write, all)', undefined, types.string)
    .addOptionalParam('showVmDetails', 'Show VM details (none, all)', undefined, types.string)
    .addOptionalParam('showGasDetails', 'Show gas details (none, all)', undefined, types.string)
    .addFlag('showCalls', 'Print more detailed call traces')
    .addFlag('resolveHashes', 'Ask openchain for ABI names')
    .addFlag('force', 'Force download even if the binary already exists')
    .setAction(
        async (
            {

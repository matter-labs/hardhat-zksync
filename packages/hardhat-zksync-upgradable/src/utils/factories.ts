...
export async function getProxyArtifact(hre: HardhatRuntimeEnvironment): Promise<ZkSyncArtifact> {
    const proxyPath = (await hre.artifacts.getArtifactPaths()).find((x) => x.includes(path.sep + ERC1967_PROXY_JSON));
...
export async function getProxyFactory(
    ...
    return new ContractFactory(proxyContract.abi, proxyContract.bytecode, wallet, deploymentType);
}
...
export async function getBeaconProxyArtifact(hre: HardhatRuntimeEnvironment): Promise<ZkSyncArtifact> {
    const beaconProxyPath = (await hre.artifacts.getArtifactPaths()).find((x) =>
        x.includes(path.sep + BEACON_PROXY_JSON),
    );
...
export async function getBeaconProxyFactory(
    ...
    return new ContractFactory(beaconProxyArtifact.abi, beaconProxyArtifact.bytecode, wallet, deploymentType);
}
...
export async function getProxyAdminFactory(
    ...
    return new ContractFactory(proxyAdminContract.abi, proxyAdminContract.bytecode, wallet, deploymentType);
}

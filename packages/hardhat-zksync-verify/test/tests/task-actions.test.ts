...
import { getCompilerVersions, getConstructorArguments, getContractInfo, verify, verifyContract } from "../../src/task-actions";
...
describe("verifyContract", async function () {
  it("should call runSuper if zksync is false", async function () {
    let runSuperStub = sinon.stub().resolves(0);
    let hre = {
      network: {
        zksync: false,
      },
  ...
  it("should call run with the correct arguments if zksync is true and address is valid", async function () {
    let retriveByteCodeStub = sinon
      .stub(utils, "retrieveContractBytecode")
      .resolves("0x1234567890");
    let inferSolcVersion = sinon
      .stub(metadata, "inferSolcVersion")
      .resolves("0.8.0");
    let getSupportedCompilerVersionsStub = sinon
      .stub(service, "getSupportedCompilerVersions")
      .resolves(["0.8.0"]);
    let getSolidityStandardJsonInputStub = sinon
      .stub(plugin, "getSolidityStandardJsonInput")
      .resolves({
        language: "Solidity",
        sources: {
    ...
    let verifyContractRequestStub = sinon
    ...
    expect(hre.run.thirdCall.args[0]).to.equal(
      TASK_VERIFY_GET_CONTRACT_INFORMATION,
    );
    expect(hre.run.getCall(3).args[0]).to.equal(
      TASK_COMPILE_SOLIDITY_GET_DEPENDENCY_GRAPH,
    );
    expect(hre.run.getCall(4).args[0]).to.equal(
      TASK_CHECK_VERIFICATION_STATUS,
    );
  });
});
describe("getCompilerVersions", async function () {
  it("should call runSuper if zksync is false", async function () {
    let runSuperStub = sinon.stub().resolves([]);
    let hre = {
      network: {
        zksync: false,
      },
...
describe("verify", async function () {
  ...
  await verify({
    ...
    libraries: "",
    noCompile: false,
  }, hre as any, runSuperStub as any);

  expect(runSuperStub.calledOnce).to.be.true;
});

  it("should throw an error if address is undefined", async function () {
    let libraries = sinon.stub(plugin, "getLibraries").resolves({});
    let runSuperStub = sinon.stub().resolves(0);
    let hre = {
      network: {
        zksync: true,
...
describe("getConstructorArguments", async function () {
  ...
  it("should throw an error if importing constructorArgsModule fails", async function () {
    ...
    const runSuperStub = sinon.stub().resolves();
    const importStub = sinon.stub(utils, 'extractModule').throws(new Error("Import error"));
...
describe("getContractInfo", async function () {
  ...
  it("should throw an error if contractFQN is undefined", async function () {
    ...
    const runSuperStub = sinon.stub().resolves({});
    const inferContractArtifactsStub = sinon.stub(plugin, 'inferContractArtifacts').throws(new Error("contractFQN is undefined"));
  ...
  it("should throw an error if no matching contract is found", async function () {
    ...
    const hre = {
      artifacts: {
        getAllFullyQualifiedNames : sinon
        .stub()
        .resolves(["contracts/Contract.sol:Contract"]),
    ...
    const runSuperStub = sinon.stub().resolves({});
    const extractMatchingContractInformationStub = sinon.stub(bytecode, 'extractMatchingContractInformation').resolves();
  ...
  it("should return contract information if contractFQN is defined and matching contract is found", async function () {
    ...
    const runSuperStub = sinon.stub().resolves({});
    const extractMatchingContractInformationStub = sinon.stub(bytecode, 'extractMatchingContractInformation').resolves(contractInformation);

...
export async function getSigner(hre: HardhatRuntimeEnvironment, address: string): Promise<HardhatZksyncSigner> {
    return hre.ethers.providerL2.getSigner(address) as HardhatZksyncSigner;
}
...
export function makeGetContractFactory(hre: HardhatRuntimeEnvironment): GetContractFactory {
    return async function (
        ...args: Parameters<GetContractFactoryArtifact | GetContractFactoryArtifactName | GetContractFactoryAbiBytecode>
    ): Promise<ContractFactory> {
        if (isArtifact(args[0])) {
            return getContractFactoryFromArtifact(hre, ...(args as Parameters<GetContractFactoryArtifact>));
        }

        if (args[0] instanceof Array && ethers.utils.isBytesLike(args[1])) {
            return getContractFactoryByAbiAndBytecode(hre, ...(args as Parameters<GetContractFactoryAbiBytecode>));
        }
...
async function getContractFactoryByAbiAndBytecode(
    ...
    return new ContractFactory(abi, bytecode, walletOrSigner, deploymentType);
}

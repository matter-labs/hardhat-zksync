...
async function main() {
    ...
    const box = await hre.zkUpgrades.deployProxy(deployer.zkWallet, contract, [42], { initializer: 'initialize', unsafeAllow: ['state-variable-assignment']});

    await box.waitForDeployment();

    // upgrade proxy implementation
    const BoxUupsV2 = await deployer.loadArtifact('BoxUupsV2');
    const upgradedBox = await hre.zkUpgrades.upgradeProxy(deployer.zkWallet, await box.getAddress(), BoxUupsV2, { unsafeAllow: ['state-variable-assignment'] });

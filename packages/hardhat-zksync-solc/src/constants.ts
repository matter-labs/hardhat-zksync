...
export const COMPILER_VERSION_RANGE_ERROR = (version: string, minVersion: string, latestVersion: string) => `Invalid zksolc compiler version ${version}. Valid versions are ${minVersion} to ${latestVersion}`;
export const COMPILER_VERSION_WARNING = (version: string, latestVersion: string) => `zksolc compiler version ${version} is not the latest. We recommend using the latest version ${latestVersion}.`;
export const COMPILER_BINARY_CORRUPTION_ERROR = 'Downloaded zksolc compiler binary is corrupted';

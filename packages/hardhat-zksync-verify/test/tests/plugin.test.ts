...
describe("Plugin", () => {
  ...
  let extractMatchingContractInformationStub: sinon.SinonStub;
  ...
  let bytecode: Bytecode = new Bytecode("0x1234567890");

  describe("inferContractArtifacts", () => {
    it("should throw ZkSyncVerifyPluginError when no matching contract is found", async () => {
      extractMatchingContractInformationStub = sinon
        .stub(bytecodes, "extractMatchingContractInformation")
        .returns(
    ...
    it("should return the contract information when a matching contract is found", async () => {
      ...
      artifacts.getAllFullyQualifiedNames = sinon
        .stub()
      ...
      expect(contractInformation.contractOutput.evm.bytecode.object).to.equal(
        "0x1234567890",
      );
    });
  });
...
Instead, this name was received: ${contractFQN}`);
  ...
  describe("getSolidityStandardJsonInput", async function () {
    ...
    it("should return the Solidity standard JSON input", async function () {
      ...
      const solidityStandardJsonInput = getSolidityStandardJsonInput(
        this.env,
        resolvedFiles,
      );
      ...
      expect(
        solidityStandardJsonInput.sources["contracts/Contract.sol"].content,
      ).to.equal("contract Contract {}");
      expect(solidityStandardJsonInput.settings.optimizer.enabled).to.be.true;
      expect(solidityStandardJsonInput.settings.areLibrariesMissing).to.be
        .false;
    });
  });

import { ValidationData, StorageLayout, stabilizeTypeIdentifier, UpgradesError } from '@openzeppelin/upgrades-core';
import { isCurrentLayoutVersion } from '@openzeppelin/upgrades-core/dist/storage/extract';
import { isStructMembers } from '@openzeppelin/upgrades-core/dist/storage/layout';
import {
    findVersionWithoutMetadataMatches,
    unfoldStorageLayout,
} from '@openzeppelin/upgrades-core/dist/validate/query';
import { DeepArray, deepEqual } from '@openzeppelin/upgrades-core/dist/utils/deep-array';
    ...
    if (isCurrentLayoutVersion(layout)) {
        ...
        if (updatedLayout === undefined) {
            ...
            await manifest.lockedRun(async () => {
                const innerData = await manifest.read();
                // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
                innerData.impls[versionWithoutMetadata]!.layout = updatedLayout;
                await manifest.write(innerData);
            });

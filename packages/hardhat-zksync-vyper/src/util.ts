...
export function assertPluginInvariant(invariant: boolean, message: string): asserts invariant {
    if (!invariant) {
        throw new VyperPluginError(message);
    }
}

export function normalizeVyperConfig(vyperConfig: VyperUserConfig): MultiVyperConfig {
...
export function getArtifactFromVyperOutput(sourceName: string, output: ContractOutput): Artifact {

...
export async function deploy(
    ...
    const deploymentEntry = await loadCache(
        ...
        overrides?.customData?.salt ?? ethers.constants.HashZero,
        factoryDeps,
    );
    ...
    await saveCache(hre, artifact, {
        ...
        salt: overrides?.customData?.salt ?? ethers.constants.HashZero,
        deploymentType,
        factoryDeps,
 ...
 */
async function extractFactoryDeps(hre: HardhatRuntimeEnvironment, artifact: ZkSyncArtifact): Promise<string[]> {
...
function _createDefaultEthProvider(): ethers.providers.BaseProvider {
